...
String WiFiProvisioning::_formHTML() {
    char buf[512];
    snprintf_P(buf, sizeof(buf), P_FORM, htmlEscape(_ssid).c_str());
    return String(buf);
}
...
void WiFiProvisioning::_buildScanCache(int n) {
    ...
    for (int i = 0; i < n && shown < 15; i++) {
        ...
        String bars = signalBars(quality);
        char row[384];
        snprintf_P(row, sizeof(row), P_NET_ROW,
            htmlEscape(ssid).c_str(), htmlEscape(ssid).c_str(), bars.c_str());
        html += row;
        shown++;
    }
